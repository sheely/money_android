...
public class CxExecuteInfoActivity extends BaseActivity {
    ...
    BasicSingleItem executeInfoItem, startTimeItem, endTimeItem;
    EditText locationEditText, budgetEditText, remarkEditText;

    void setupView() {

        executeInfoItem = (BasicSingleItem) findViewById(R.id.execute_item);
        findViewById(R.id.look_execute).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                startActivity("cp://cydetail?friendid=" + dsExeInfo.getString("oppoexecuterid"));
            }
        });
    ...
    void updateView() {
        ...
        executeInfoItem.setSubTitle(dsExeInfo.getString("oppoexecutor"));
        startTimeItem.setSubTitle(Utils.formate(dsExeInfo.getString("startTime")));
        endTimeItem.setSubTitle(Utils.formate(dsExeInfo.getString("endTime")));
        locationEditText.setText(dsExeInfo.getString("executePlace"));
        budgetEditText.setText(dsExeInfo.getString("budget"));
        remarkEditText.setText(dsExeInfo.getString("remark"));
    }
    ...
    public void onTaskFinished(SHTask task) throws Exception {
        if (task == queryTask) {
            ...
            dsExeInfo = DSObjectFactory.create("cx_execute_info").fromJson(task.getResult());
            updateView();
    ...
    private void updateDisplay() {
        if (currentDialogId == START_DATE_DIALOG_ID) {
            startTimeItem.setSubTitle(Utils.formate(new StringBuilder().append(mYear).append("-").append(
                mMonth + 1).append("-").append(mDay).toString()));
        } else if (currentDialogId == END_DATE_DIALOG_ID) {
            endTimeItem.setSubTitle(Utils.formate(new StringBuilder().append(mYear).append("-").append(
                mMonth + 1).append("-").append(mDay).toString()));
        }
    }
